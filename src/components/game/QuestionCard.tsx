...
export function QuestionCard({ question, category, myScore, opponentScore, onAnswer, timeLimitMs = 10000 }: QuestionCardProps) {
  ...
  return (
    <div className="w-full max-w-md flex flex-col h-full animate-in fade-in duration-500">
      ...
      {/* Top Scores */}
      <div className="flex items-center justify-center space-x-6 space-x-reverse text-white mb-6 font-bold text-sm shadow-text">
      ...
      <div className="flex-1 flex flex-col">
        ...
        <div className="bg-white rounded-[2rem] pt-8 pb-6 px-6 relative shadow-lg mb-4 flex-1 flex flex-col items-center justify-center min-h-[200px]">
          {/* Category Badge */}
          <div className="absolute -top-4 left-1/2 -translate-x-1/2 bg-[#e61a4b] text-white px-6 py-1.5 rounded-full font-bold text-sm shadow-md">
          ...
          {/* Tag (like Frnz69 in screenshot) */}
          <div className="absolute bottom-4 bg-slate-300 text-slate-600 px-6 py-1 rounded-full text-xs font-bold shadow-inner">
            طراح سوال: <span className="text-[#1a6ebd]">{question.authorName || 'ویستا'}</span>
          </div>
        </div>

        {/* Timer Bar & Pause */}
        <div className="flex items-center space-x-2 space-x-reverse mb-6">
          <button disabled title="به زودی" className="bg-[#2a87d0]/60 border border-[#1b73b5] shadow-[0_4px_0_#1b73b5] rounded-xl p-2 flex flex-col items-center justify-center w-14 h-14 cursor-not-allowed opacity-70">
          ...
          <div className="flex-1 bg-[#1b73b5] h-8 rounded-full border border-[#114b82] p-1 shadow-inner relative overflow-hidden">
            <div 
              className="h-full bg-[#20b2f5] rounded-full transition-all duration-75 ease-linear"
        ...
        <div className="grid grid-cols-2 gap-3 mb-6">
          {question.options.map((option, index) => {
            ...
            if (showCorrect) {
              ...
              btnClass = "bg-[#e61a4b] text-white shadow-[0_4px_0_#b3153b] font-black animate-shake";
