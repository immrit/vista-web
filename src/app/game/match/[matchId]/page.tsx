      ...
      <div className="min-h-screen bg-[#4c1d95] font-sans flex flex-col relative overflow-hidden">
        ...
        <div className="flex-1 flex flex-col items-center justify-center px-6 relative z-10 space-y-8 py-10">
          ...
          <div className="text-center space-y-2">
            ...
            <div className="flex flex-col items-center space-y-1 mt-2 bg-[#3b0764]/40 rounded-xl p-3 border border-white/10 shadow-inner">
              ...
              <div className="text-[#d8b4fe] font-bold text-lg flex items-center space-x-1 space-x-reverse">
          ...
          {/* Scoreboard */}
          <div className="bg-[#3b0764]/60 backdrop-blur-sm rounded-3xl p-6 w-full max-w-xs border border-white/20 shadow-xl">
          ...
          <div className="w-full max-w-xs space-y-2">
            {rounds.filter(r => r !== null).map((round, idx) => (
              <div key={idx} className="bg-[#3b0764]/40 rounded-2xl px-4 py-2 flex items-center justify-between">
          ...
          <div className="w-full max-w-xs space-y-3 pb-8">
            ...
            <button
              onClick={() => router.push('/game')}
              className="w-full bg-[#3b0764] text-white/70 font-bold py-3 rounded-2xl border border-white/20 hover:bg-[#4c1d95] transition-colors"
            >
              بازگشت به خانه
            </button>
          </div>
        ...
        <div className="flex justify-between items-start">
          ...
          <div className="flex flex-col items-center relative">
            ...
            <div
              ...
              className="w-16 h-16 rounded-full border-2 border-[#a855f7] overflow-hidden bg-white shadow-lg relative z-10 cursor-pointer hover:border-white transition-all active:scale-95"
            >
              <img src={me.avatarUrl} alt="Me" className="w-full h-full object-cover" />
            </div>
            <span className="text-white text-xs font-bold mt-2 bg-[#3b0764] px-3 py-1 rounded-full shadow-inner">{me.name}</span>
          </div>

          {/* Scores */}
          <div className="flex items-center space-x-4 space-x-reverse mt-4">
            <div className="bg-[#3b0764] shadow-inner rounded-xl w-14 h-14 flex items-center justify-center border border-[#4c1d95]">
          ...
          <div className="flex flex-col items-center relative">
            {opponent ? (
              <>
                ...
                <span className="text-white text-xs font-bold mt-2 bg-[#3b0764] px-3 py-1 rounded-full shadow-inner">{opponent.name}</span>
              </>
            ) : (
              <>
                <div className="w-16 h-16 rounded-full border-2 border-dashed border-[#a855f7] bg-black/10 flex items-center justify-center text-white/50 shadow-inner relative z-10">?</div>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-2 relative z-10 pb-28">
        {rounds.map((round, idx) => {
          ...
          if (!hasPlayed && !isCurrentRound) {
            return (
              <div key={idx} className="h-14 bg-[#3b0764] rounded-full shadow-[inset_0_2px_4px_rgba(0,0,0,0.2)] border border-[#4c1d95] w-full flex opacity-40">
          ...
          if (isCurrentRound && round) {
            ...
            return (
              <div key={idx} className="h-14 bg-[#5b21b6] rounded-full shadow-md w-full flex items-center justify-between px-1 border border-[#7c3aed]">
                <div className="flex-1 flex justify-center items-center">
                  {myTurn ? <span className="text-white text-sm font-bold drop-shadow-sm animate-pulse">نوبت توئه ▶</span> : null}
                </div>
                <div className="w-28 h-12 bg-[#3b0764] rounded-full flex items-center justify-center px-2 text-white font-bold text-xs shadow-inner">
          ...
          if (hasPlayed && round) {
            ...
            const renderCircles = (answers: number[], times: number[]) => {
              ...
              for (let i = 0; i < 3; i++) {
                ...
                if (!isAnswerPending(ans, time) && question && ans === question.correctOptionIndex)
                  circles.push(<div key={i} className="w-5 h-5 rounded-full bg-[#78c02c] shadow-[inset_0_-2px_0_rgba(0,0,0,0.2)]"></div>);
                else if (!isAnswerPending(ans, time) && question)
                  circles.push(<div key={i} className="w-5 h-5 rounded-full bg-[#ea4b34] shadow-[inset_0_-2px_0_rgba(0,0,0,0.2)]"></div>);
                else
                  circles.push(<div key={i} className="w-5 h-5 rounded-full bg-[#3b0764] shadow-inner"></div>);
              }
              return circles;
            };

            return (
              <div key={idx} className="h-14 bg-[#3b0764] rounded-full shadow-[inset_0_2px_4px_rgba(0,0,0,0.2)] border border-[#4c1d95] w-full flex items-center justify-between px-3">
        ...
        <button
          ...
          className={cn(
            ...
            canPlay
              ...
              : "bg-[#3b0764] text-white/30 border-b-4 border-[#2e1065]"
          )}
        >
