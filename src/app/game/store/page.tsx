...
// Fallback only; the authoritative list comes from GET /v1/payment/packages.
const FALLBACK_PACKAGES: CoinPackage[] = [
  { id: 'coins_1000', coins: 1000, bonus: 0, priceToman: 10000 },
  { id: 'coins_5000', coins: 5000, bonus: 500, priceToman: 45000 },
  { id: 'coins_12000', coins: 12000, bonus: 2000, priceToman: 90000 },
];
...
export default function GameStorePage() {
  ...
  useEffect(() => {
    apiClient
      .get<{ packages: CoinPackage[] }>('/v1/payment/packages')
      .then((res) => {
        if (res?.packages?.length) setPackages(res.packages);
      })
      .catch((e) => console.error('Failed to load packages, using fallback:', e));
  }, []);
              ...
              <div key={pkg.id} className="bg-white rounded-2xl p-1 shadow-[0_4px_0_rgba(0,0,0,0.1)] relative overflow-hidden flex items-stretch">
                ...
                <div className="w-24 bg-slate-50 flex items-center justify-center rounded-r-xl border-l border-slate-100 p-2 relative">
                   <div className="w-12 h-12 relative">
                     <Image src="/images/coin.png" alt="coins" fill className="object-contain drop-shadow-sm" />
                   </div>
                </div>

                <div className="flex-1 p-4 flex flex-col justify-center">
                  <div className="font-black text-xl text-slate-800 flex items-center space-x-1 space-x-reverse">
                    <span>{(pkg.coins + pkg.bonus).toLocaleString('fa-IR')}</span>
                    <span className="text-sm text-slate-500">سکه</span>
                  </div>
                  <div className="text-[#78c02c] font-bold text-lg mt-1">
                    {pkg.priceToman.toLocaleString('fa-IR')} <span className="text-xs">تومان</span>
                  </div>
                </div>
