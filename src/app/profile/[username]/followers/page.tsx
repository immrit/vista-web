...
export default function FollowersPage() {
  ...
  const load = useCallback(async (reset = false) => {
    try {
      ...
      const result = await profileApi.getFollowers(profile.id, 30, reset ? undefined : cursor)
  ...
  return (
    <div className="min-h-screen bg-vista-bg dark:bg-vista-bg-dark">
      <MobileTopBar title={`دنبال‌کنندگان @${username}`} showLogo={false} showNotifications={false}
        rightAction={
      ...
      <div className="feed-container py-4">
        <h1 className="hidden lg:block text-xl font-bold px-4 mb-4">دنبال‌کنندگان @{username}</h1>
        {isLoading ? (
          <div className="flex justify-center py-12"><Loader2 className="w-8 h-8 animate-spin text-vista-primary" /></div>
        ) : users.length === 0 ? (
          <div className="text-center py-16 text-vista-text-secondary">هنوز دنبال‌کننده‌ای ندارد</div>
        ) : (
