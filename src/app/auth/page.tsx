...
function getAuthRedirectPath(user: AuthUser, fallback: string) {
    if (user.password_required) return "/set-password"
...
function describeError(error: unknown, fallback = "خطایی رخ داد. دوباره تلاش کنید.") {
    ...
    if (raw.includes("AUTH_HAS_PASSWORD")) return "این حساب رمز عبور دارد. لطفاً با رمز عبور وارد شوید."
...
export default function AuthPage() {
    ...
    const completeAuth = (auth: AuthResponse, overrideRedirect?: string) => {
        router.replace(overrideRedirect || getAuthRedirectPath(auth.user, redirectPath))
    }
    ...
    const requestOtp = async (phoneNumber = normalizedIdentifier, isResend = true) => {
        ...
        try {
            ...
            if (raw.includes("AUTH_HAS_PASSWORD")) {
                setStep("password")
                setNotice("برای این حساب رمز عبور تنظیم شده است.")
                setPassword("")
                return
        ...
        try {
            ...
            if (response.auth) {
                if (isRegistering && password) {
                    ...
                    // This means they do NOT have a password. Force them to set one.
                    completeAuth(response.auth, "/set-password")
                }
                return
            }
